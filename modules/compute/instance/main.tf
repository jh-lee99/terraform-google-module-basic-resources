...
resource "google_compute_instance" "default" {
  ...
  boot_disk {
    initialize_params {
      ...
      size  = var.disk_size
      type  = var.boot_disk_type # "pd-standard" 또는 pd-balanced, pd-ssd
    }
  }
  ...
  metadata_startup_script = var.startup_script # file("./userdata/user_data.sh")

  metadata = {
    ssh-keys = "${var.gce_ssh_user}:${file(var.gce_ssh_pub_key_file)}" # 사용자:공개키
  }
